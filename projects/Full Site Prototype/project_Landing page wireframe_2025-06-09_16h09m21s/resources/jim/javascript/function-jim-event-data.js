...
(function(window, undefined) {
  ...
  jQuery.extend(jimEvent.fn, {
    ...
    "jimUpdateData": function(args, callback) {
      var self = this, datamaster, instances, instance, i, iLen, j, jLen, variable, records, record, value, $dataViews, g, gLen, k, kLen, $datarows, field, $fields, f, fLen;
      if (args && args.fields) {
        /* update data */
              ...
              if($rowOrGridCell) {
                for (field in instance.userdata) {
                  if (instance.userdata.hasOwnProperty(field)) {
                    ...
                    $fields = $rowOrGridCell.find("[name='" + field + "']");
                    for(f=0, fLen=$fields.length; f < fLen; f += 1) {
            			var $field = jQuery($fields[f]);
            			$field.val(value);
