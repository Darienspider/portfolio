        ...
        if(jimUtil.exists(undoStack)) {
          for(c=0, cLen=undoStack.length; c<cLen; c+=1) {
          	for(iAction=0, iActionLen=undoStack[c].action.length; iAction<iActionLen; iAction+=1) {
	            ...
	            if(undoAction.action && undoAction.parameter && (undoType == self.event.type || (undoType == "mouseenter" || undoType == "dragenter" || undoType == "mouseover") && (self.event.type == "mouseleave" || self.event.type == "dragleave") )) {
          ...
          for(s=0, len=action.parameter.length; s<len; s+=1) {
			...
			for(t=0, tLen=style.target.length; t<tLen; t+=1) {
                ...
                if($target) {
                  //check if shape
                  bShape=false;
                  if($target.jimGetType() === itemType.shapewrapper && !("opacity" in style.attributes)){
                ...
                var opacity = $target.find("path, rect, circle, ellipse, line, polyline, polygon, use").not("defs *").attr("opacity");
                if (opacity == undefined) {
                  opacity = 1.0;
                }
                undoStyle.attributes[property] = opacity;
              }
              else if (jimUtil.isPathProperty(property)) {
                var pathStyleValue = undefined;
                var paths = $target.find("path, rect, circle, ellipse, line, polyline, polygon, use").not("defs *");
                if (property == "path-stroke") {
                  if ($(paths).css("stroke") != undefined) {
                    pathStyleValue = jimUtil.rgbToHex($(paths).css("stroke"));
                  } 
                  else {
                    pathStyleValue = $(paths).attr("stroke");
                  }
                }
                else if (property == "path-stroke-width") {
                  if ($(paths).css("stroke-width") != undefined) {
                    pathStyleValue = $(paths).css("stroke-width") + "";
                  }
                  else {
                    pathStyleValue = $(paths).attr("stroke-width");
                  }
                }
                else if (property == "path-stroke-dasharray") {
                  if ($(paths).css("stroke-dasharray") != undefined) {
                    pathStyleValue = $(paths).css("stroke-dasharray");
                  }
                  else {
                    pathStyleValue = $(paths).attr("stroke-dasharray");
                  }
                  if (pathStyleValue != undefined) {
                    var dashType = "solid";
                    pathStyleValue = pathStyleValue.replaceAll("px", "").replaceAll(",", "");
                    var values = pathStyleValue.split(" ");
                    if (values.length == 2) {
                      if (values[0] == "0.1") {
                        dashType = "dotted";
                      }
                    ...
                    pathStyleValue = dashType;
                  }
                }
                else if (property == "path-background-color") {
                  if ($(paths).css("fill") != undefined) {
                    pathStyleValue = jimUtil.rgbToHex($(paths).css("fill"));
                  }
                  else {
                    pathStyleValue = $(paths).attr("fill");
                  }
                }
                else if (property == "path-background-opacity") {
                  if ($(paths).css("fill-opacity") != undefined) {
                    pathStyleValue = $(paths).css("fill-opacity") + "";
                  }
                  else {
                    pathStyleValue = $(paths).attr("fill-opacity");
                  }
                }
                else if (property == "stroke-cap") {
                  if ($(paths).css("stroke-linecap") != undefined) {
                    pathStyleValue = $(paths).css("stroke-linecap") + "";
                  }
                  else {
                    pathStyleValue = $(paths).attr("stroke-linecap");
                  }
                }
                else if (property == "path-background-image") {
                  // TO DO
                }
                undoStyle.attributes[property] = pathStyleValue;
              }
                  	else
                  	  undoStyle.attributes[property] = $target.css(property);
                    }
